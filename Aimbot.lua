...
task.spawn(function()
    while task.wait(0.1) do
        if _G.KnifeAura and LocalPlayer.Character then
            local tool = LocalPlayer.Character:FindFirstChildOfClass("Tool")
            if tool and (tool.Name:lower():find("knife") or tool.Name:lower():find("bayonet")) then
                for _, player in pairs(Players:GetPlayers()) do
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                -- Team Check ESP filter
                local shouldShow = true
                if _G.TeamCheck and player.Team == LocalPlayer.Team then shouldShow = false end

                -- ESP Highlight
                local high = char:FindFirstChild("GeminiHigh")
                if _G.EspEnabled and shouldShow then
